...
public class ${name}Procedure {
  ...
  private static void setFog(FogShape shape, double start, double end, double color0, double color1, double color2, double color3, String colorModel) {
    if (eventType.equals(RENDER_FOG)) {
      EntityViewRenderEvent.RenderFogEvent event = (EntityViewRenderEvent.RenderFogEvent) param.get("RenderFog");
    ...
    if (eventType.equals(COMPUTE_FOG_COLOR) || eventType.equals(RENDER_SKY)) {
      ...
      if (eventType.equals(COMPUTE_FOG_COLOR)) {
        EntityViewRenderEvent.FogColors event = (EntityViewRenderEvent.FogColors) param.get("ComputeFogColor");
  ...
  <#-- Render Fog #-->
	@SubscribeEvent
  public static void onRenderingFog(EntityViewRenderEvent.RenderFogEvent event) {
    if (isDrawable()) {
      Minecraft minecraft = Minecraft.getInstance();
      eventType = RENDER_FOG;
      param.put("RenderFog", event);
      param.put("Camera", minecraft.gameRenderer.getMainCamera());
      param.put("PartialTick", minecraft.getFrameTime());
  ...
  <#-- Compute Fog Color #-->
	@SubscribeEvent
  public static void onComputingFogColor(EntityViewRenderEvent.FogColors event) {
    if (isDrawable()) {
      Minecraft minecraft = Minecraft.getInstance();
	...
	@SubscribeEvent
  public static void onRenderingSky(TickEvent.RenderTickEvent event) {
    if (Minecraft.getInstance().level != null) {
      if (isDrawable()) {
        <#-- Render Sky #-->
        Minecraft.getInstance().level.effects().setSkyRenderHandler((ticks, partialTick, poseStack, clientLevel, minecraft) -> {
          eventType = RENDER_SKY;
          param.put("Camera", minecraft.gameRenderer.getMainCamera());
          param.put("PoseStack", poseStack);
          param.put("Matrix4f", RenderSystem.getProjectionMatrix());
          param.put("PartialTick", partialTick);
          execute(event<#if dependenciesCode?has_content>,</#if>${dependenciesCode});
        });
        <#-- Render Clouds #-->
        Minecraft.getInstance().level.effects().setCloudRenderHandler((ticks, partialTick, poseStack, clientLevel, minecraft, camX, camY, camZ) -> {
