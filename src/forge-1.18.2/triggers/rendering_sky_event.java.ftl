...
@Mod.EventBusSubscriber public class ${name}Procedure {
  ...
  private static void setFogColor(double color0, double color1, double color2, double color3, String component) {
    if (eventType.equals(COMPUTE_FOG_COLOR) || eventType.equals(RENDER_SKY)) {
      ...
      if (eventType.equals(COMPUTE_FOG_COLOR)) {
        EntityViewRenderEvent.FogColors event = (EntityViewRenderEvent.FogColors) renderingManager.get("ComputeFogColor");
  ...
  private static void setFogDistance(double start, double end) {
    if (eventType.equals(RENDER_FOG)) {
      EntityViewRenderEvent.RenderFogEvent event = (EntityViewRenderEvent.RenderFogEvent) renderingManager.get("RenderFog");
  ...
  public static void onRenderingFog(EntityViewRenderEvent.RenderFogEvent event) {
    ...
    renderingManager.put("PartialTick", minecraft.getFrameTime());
  ...
  public static void onRenderingSky(TickEvent.RenderTickEvent event) {
    if (Minecraft.getInstance().level != null) {
      ...
      Minecraft.getInstance().level.effects().setSkyRenderHandler((ticks, partialTick, poseStack, clientLevel, minecraft) -> {
        ...
        renderingManager.put("Camera", minecraft.gameRenderer.getMainCamera());
        renderingManager.put("PoseStack", poseStack);
        renderingManager.put("Matrix4f", RenderSystem.getProjectionMatrix());
        renderingManager.put("PartialTick", partialTick);
        execute(event<#if dependenciesCode?has_content>,</#if>${dependenciesCode});
      });
      <#-- Render Clouds #-->
