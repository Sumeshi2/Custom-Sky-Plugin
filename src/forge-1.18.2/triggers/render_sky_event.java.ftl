...
public class ${name}Procedure {
      ...
      if (!immutability && rainLevel <= 0) {
        ...
        RenderSystem.setShaderTexture(0, new ResourceLocation(id, path));
            ...
            int index = i - posX + 16 + (j - posZ + 16) * 32;
						...
						if (minY != maxY) {
							Random random = new Random((long) (i * i * 3121 + i * 45238971 ^ j * j * 418711 + j * 13761));
              ...
              if (renderType.equals(STORM)) {
                ...
                speedFactor = 10.0F;
							...
							if (renderType.equals(RAIN) && flag1) {
							  ...
							  float float0 = -(int0 + partialTick) / 32.0F * (3.0F + random.nextFloat());
							  ...
							  float float1 = (float) (random.nextDouble() + factor * speedFactor * random.nextGaussian() * 0.01D);
							  float float2 = (float) (random.nextDouble() + factor * speedFactor * random.nextGaussian() * 0.001D);
							  mutableBlockPos.set(i, blockPosY, j);
  ...
  private static void setWeatherParticles(ParticleOptions normal, ParticleOptions vapor, net.minecraft.sounds.SoundEvent sound, boolean biomeEffect, boolean immutability) {
    if (eventType.equals(SET_WEATHER_PARTICLES)) {
      ...
      if (flag0) {
        ...
        if (minecraft.options.particles == ParticleStatus.DECREASED) {
          particleStatus = 2;
        } else {
          particleStatus = 1;
        }
        ...
        for (int i = 0; i < amount; i++) {
          int random0X = random.nextInt(21) - 10;
          int random0Z = random.nextInt(21) - 10;
          ...
          if (pos2Y > levelReader.getMinBuildHeight() && pos2Y <= pos0Y + 10 && pos2Y >= pos0Y - 10) {
            ...
            if (flag1) {
              ...
              double random1X = random.nextDouble();
              double random1Z = random.nextDouble();
        ...
        if (blockPos1 != null && random.nextInt(3) == 0) {
  ...
  private static boolean isRenderable() {
    String modid = Minecraft.getInstance().level.dimension().location().getNamespace();
    boolean flag = false;
  ...
  private static int[] getTextureSize(String id, String path) {
    ...
    try {
      InputStream stream = Minecraft.getInstance().getResourceManager().getResource(textureLocation).getInputStream();
  ...
  private static double getFogColor(String component) {
    ...
    if (!eventType.equals(NONE)) {
      ...
      if (component.equals(RED) || component.equals(GREEN) || component.equals(BLUE) || component.equals(ALPHA)) {
  ...
  public static void onComputeFogColor(EntityViewRenderEvent.FogColors event) {
	...
	@SubscribeEvent
  public static void onRenderLevel(TickEvent.RenderTickEvent event) {
    if (Minecraft.getInstance().level != null) {
      Minecraft minecraft = Minecraft.getInstance();
      DimensionSpecialEffects effects = minecraft.level.effects();

      if (isRenderable()) {
        <#-- Render Sky #-->
        effects.setSkyRenderHandler((_ticks, _partialTick, _poseStack, _clientLevel, _minecraft) -> {
          eventType = RENDER_SKY;
          camera = _minecraft.gameRenderer.getMainCamera();
          poseStack = _poseStack;
          projectionMatrix = RenderSystem.getProjectionMatrix();
          partialTick = _partialTick;

          execute(event<#if dependenciesCode?has_content>,</#if>${dependenciesCode});
          eventType = NONE;
        });
        <#-- Render Clouds #-->
        effects.setCloudRenderHandler((_ticks, _partialTick, _poseStack, _clientLevel, _minecraft, _camX, _camY, _camZ) -> {
        ...
        <#-- Render Weather #-->
        effects.setWeatherRenderHandler((_ticks, _partialTick, _clienLevel, _minecraft, _lightTexture, _camX, _camY, _camZ) -> {
        ...
        effects.setWeatherParticleRenderHandler((_ticks, _clientLevel, _minecraft, _camera) -> {
